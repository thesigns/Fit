...
public class Weight : Command
{
    ...
    public override string Execute(List<string> args, Repo repo)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"Previous weight: {previous.weight.GetValue(Mass.Unit.Kilogram)} kg ({Units.TicksToDate(previous.tick)})");
            ...
            Console.WriteLine($"New weight: {Math.Round(weight.GetValue(Mass.Unit.Kilogram), 1)} kg [{difference} kg] ({Units.TicksToDate(DateTime.UtcNow.Ticks)})");
        }
        catch(Exception e)
        {
